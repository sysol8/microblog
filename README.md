...
- `alertStore` предлагает крайне удобный способ менеджмента уведомлений. Использование:
  ...
  const like = async () => {
    if (!user) {
      alertConfig.auth.onAuthRequired(); // обращаемся к конфигу, выбираем группу ошибок, 
      return;                            // вызываем алерт с конкретной ошибкой
    }
...
2. Развернуть проект через Docker Compose
    ```bash
...
## Storybook
Для проекта собрана библиотека компонентов в Storybook
...
## Что можно добавить и/или изменить
### 1. UI/UX
- Общие улучшения дизайна и верстки;
- Добавить авторизацию в модальном окне без редиректа на отдельную страницу;
- Добавить скелетоны для компонентов и страниц, зависящих от серверных данных;
### 2. Техническая реализация
- Добавить локальный стор для хранения постов;
- Осуществить миграцию на JS-фреймворк для бэкенда (Nest.js) для работы в единой экосистеме;
- Заменить SQLAlchemy на более современную ORM;
